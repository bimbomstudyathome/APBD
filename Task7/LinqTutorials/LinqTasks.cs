...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(arg => arg.Job == "Backend programmer");
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.Where(arg => arg.Job == "Frontend programmer" && arg.Salary > 1000).OrderByDescending(arg => arg.Ename);
            return result;
        }
        ...
        {
            int result = Emps.Max(arg => arg.Salary);
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.Where(arg => arg.Salary == Emps.Max(arg => arg.Salary));
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(arg => new
            {
                Nazwisko = arg.Ename,
                Praca = arg.Job
            });
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(arg => arg.Job).Select(arg => new
            {
                Praca = arg.Key,
                LiczbaPracownikow = arg.Count()
            });
            return result;
        }
        ...
        {
            bool result = Emps.Any(arg => arg.Job == "Backend programmer");
            return result;
        }
        ...
        {
            Emp result = Emps.OrderBy(arg => arg.HireDate).First(arg => arg.Job == "Frontend programmer");
            return result;
        }
        ...
        {
            IEnumerable<object> result = Depts.Join(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, emp) => dept).GroupBy(dept => dept.Deptno).Where(arg => arg.Count() > 1).Select(arg => new
            {
                name = arg.First().Dname,
                numOfEmployees = arg.Count()
            });
            return result;
        }
        ...
        {
            int result = arr.GroupBy(arg => arg).Where(arg => arg.Count() % 2 != 0).Select(arg => arg.Key).First();
            //result=
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.Where(dept => Emps.Count(emp => emp.Deptno == dept.Deptno) == 0 || Emps.Count(emp => emp.Deptno == dept.Deptno) == 5).OrderBy(dept => dept.Dname).ToList();
            //result =
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Where(arg => arg.Job.Contains("A")).GroupBy(arg => arg.Job).Where(arg => arg.Count() > 2).OrderByDescending(arg => arg.Count()).Select(arg => new
            {
                Praca = arg.Key,
                LiczbaPracownikow = arg.Count()
            });
